;;;; core/sdl-bindings.lisp -- CFFI bindings for SDL3 and SDL_ttf
;;;;
;;;; This module provides Common Lisp bindings for the SDL3 multimedia
;;;; library and SDL_ttf for font rendering.
...
(defun create-window-and-renderer (title width height flags)
  (flet ((get-flag (kw)
           (or (getf +window-flags+ kw)
               (error "Unknown flag passed to CREATE-WINDOW-AND-RENDERER: ~a" kw))))
...
(defun init (flags)
  (flet ((get-flag (kw)
           (or (getf +init-flags+ kw)
               (error "Unknown flag passed to INIT: ~a" kw))))
...
(cffi:defcfun ("SDL_StartTextInput" start-text-input) :bool)
(export 'start-text-input)

(cffi:defcfun ("SDL_StopTextInput" stop-text-input) :bool)
